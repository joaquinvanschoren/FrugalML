...
for (i in 1:quantityDataSets) {
    ...
    processedDataSet <- frugalityScoreData(hugeMatrix, dataSet, p.w = w, valOfAlgs = numOfAlgs) 
...
postProcessingEmptyValues <- FALSE 
if (postProcessingEmptyValues) {
    hugeMatrix[is.na(hugeMatrix)] <- -1 
    saveRDS(hugeMatrix, paste("objects/hugeMatrixWithProcessedValuesFrugValue", w, ".rds", sep = "")) 
} else {
    saveRDS(hugeMatrix, paste("objects/hugeMatrixOnlyWithRealValuesFrugValue", w, ".rds", sep = "")) 
}
